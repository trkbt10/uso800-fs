...
export type LlmOrchestrator = {
  ...
  /**
   * Notification-style API: request LLM to create/update the file at path.
   * The LLM implementation writes via PersistAdapter; nothing is returned.
   */
  fabricateFileContent: (path: string[], opts?: { mimeHint?: string }) => Promise<void>;
};
...
export function createLlmWebDavHooks(llm: LlmOrchestrator): WebDavHooks {
  return {
    async beforeGet({ segments, persist }) {
      // If missing or empty file, notify LLM to generate content once
      const exists = await persist.exists(segments);
      if (!exists) {
        try {
          const mimeHint = inferMimeFromPath(segments);
          await llm.fabricateFileContent(segments, { mimeHint });
        } catch (e) {
